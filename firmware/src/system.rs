...
/// A minimal system usable in panic handling contexts.
pub struct PanicSystem<StatusLed> {
    status_led: StatusLed,
    delay: Delay,
}

impl<StatusLed> PanicSystem<StatusLed>
where
    StatusLed: Indicator,
{
    pub fn from_parts(status_led: StatusLed, delay: Delay) -> Self {
        Self { status_led, delay }
    }
...
/// WindowMaster Rev1 implementation.
pub mod rev1 {
    use super::*;
    ...
    pub type PanicSystem = super::PanicSystem<indicator::ActiveLow<gpiob::PB12<Output<PushPull>>>>;
...
/// 32F072BDISCOVERY implementation.
pub mod discovery {
    pub use super::*;
    ...
    pub type PanicSystem = super::PanicSystem<indicator::ActiveLow<gpioc::PC8<Output<PushPull>>>>;

    impl PanicSystem {
        pub fn new(mut dp: Peripherals, cp: CorePeripherals, cs: &CriticalSection) -> Self {
            let mut rcc = dp.RCC.configure().freeze(&mut dp.FLASH);
            let gpioc = dp.GPIOC.split(&mut rcc);
            let status_led = indicator::ActiveLow::new(gpioc.pc8.into_push_pull_output(cs));
            let delay = Delay::new(cp.SYST, &rcc);
